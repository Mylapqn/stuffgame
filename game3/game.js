...
var fpsAverageLimit = 10;
var fpsFramesUntilAverage = 0;
...
function update(timestamp) {
	...
	if (running) {
		...
		if (fpsFramesUntilAverage >= fpsAverageLimit) {
			fpsFramesUntilAverage = 0;
			avgDeltaTime = fpsCurrentTime;
			fpsCurrentTime = 0;
		...
		//bufferTotal += connection.bufferedAmount;
		ctx.fillStyle = "white";
		ctx.textAlign = "left";
		//ctx.fillText("Total WS Buffer: " + bufferTotal + " B " + connection.binaryType, 30, 160);
		//ctx.fillText("This Frame WS Buffer: " + connection.bufferedAmount + " B " +connection.binaryType, 30, 190);
		/*
		ctx.lineWidth = 1;
